...
char* RegeditOpt::FindPath()
{
	...
	{
		DWORD dwIndex = 0, NameCnt = 0, NameMaxLen = 0;
		DWORD KeySize = 0,KeyCnt = 0,KeyMaxLen = 0,MaxDateLen = 0;
        //这就是枚举了
...
char* RegeditOpt::FindKey()
{
    ...
    if(RegOpenKeyEx(MKEY,KeyPath,0,KEY_ALL_ACCESS,&hKey)==ERROR_SUCCESS)//打开
	{
       	DWORD dwIndex = 0,NameSize = 0,NameCnt = 0, NameMaxLen = 0, Type = 0;
		...
		//&&NameSize>0
		if(NameCnt>0&& MaxDateLen >0 && NameMaxLen > 0)
		{
